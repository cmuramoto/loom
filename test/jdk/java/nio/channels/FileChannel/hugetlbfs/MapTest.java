...
/* @test
 * @bug 4429043 8002180
 * @summary Test file mapping under hugetlbfs with FileChannel
...
/**
 * Testing FileChannel's mapping capabilities with hugetlbfs.
 */
public class MapTest {
     ...
     */
    private static final Path BASE = Paths.get("/var/lib/hugetlbfs/global/pagesize-2MB");
    ...
    public static void main(String[] args) throws Throwable {
        if(!Files.exists(BASE) || !Files.isDirectory(BASE)) {
            out.printf("hugetlbfs volume %s does not exist. Aborting test.%n", BASE);
            return;
        }

        if(!HugeUtil.supports(HUGE_PS, 30)) {
            return;
        }

        blah = File.createTempFile("blah", null, BASE.toFile());

        if(!HugeUtil.usesHugeBlocks(blah, HUGE_PS)) {
            out.printf("Unsupported test. %s does no use huge blocks%n", blah);
            blah.delete();
            return;
        }
    ...
    private static void initTestFile(File blah) throws Exception {
        try {
            initTestFileWithWriter(blah);
        }catch(IOException e){
            out.printf("Write not supported: [%s]. Attempting mmap%n",e.getMessage());
            initTestFileWithMmap(blah);
        }
    }

    private static void initTestFileWithWriter(File blah) throws Exception {
    ...
    private static void initTestFileWithMmap(File blah) throws IOException {
        int max = 4000;
        int est = max*(4 + 1);

        var bb = ByteBuffer.allocate(est);

        for (int i=0; i<4000; i++) {
            var b = String.format("%04d%n", i).getBytes(ISO_8859_1);

            if(bb.remaining() < b.length) {
                var tmp = ByteBuffer.allocate(Math.max(bb.capacity() + b.length, bb.capacity() << 1));
                tmp.put(bb.flip());
                bb = tmp;
            }
        ...
        try(var fc = FileChannel.open(blah.toPath(), StandardOpenOption.READ, StandardOpenOption.WRITE)) {
            var mbb = fc.map(MapMode.READ_WRITE, 0, bb.limit());
